...
def make_datasets(device, validation: bool = True):
    ...
    datasets['train'] = PNVTrainingDataset(PARAMS.dataset_folder, PARAMS.train_file,device=device,
                                           transform=train_transform, set_transform=train_set_transform)
    if validation:
        datasets['val'] = PNVTrainingDataset(PARAMS.dataset_folder, PARAMS.val_file, device=device)
...
def make_dataloaders(device, validation=True):
    """
