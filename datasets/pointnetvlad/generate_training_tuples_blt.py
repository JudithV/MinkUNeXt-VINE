...
def construct_query_dict(df_centroids, base_path, filename, ind_nn_r, ind_r_r=12):
...
if __name__ == '__main__':
    ...
    for folder in tqdm.tqdm(folders):
        if os.path.exists(RUNS_FOLDER+"ktima/"+folder):
            with open(RUNS_FOLDER+"ktima/"+folder+"/"+FILENAME, 'w', newline='') as file:
            ...
            with open(RUNS_FOLDER+"riseholme/"+folder+"/"+FILENAME, 'w', newline='') as file:
                escritor_csv = csv.writer(file)
                escritor_csv.writerow(['timestamp','northing','easting'])
    ...
    construct_query_dict(df_train, base_path, "training_queries_blt_pruebas.pickle", ind_nn_r=5)
    construct_query_dict(df_test, base_path, "test_queries_blt_pruebas.pickle", ind_nn_r=5)
